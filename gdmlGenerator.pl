...
use strict 'vars';
use vars qw($opt_D $numRing @numDetPerRing @quartzRad @quartzZ @quartzThickness @quartzHeight @quartzWidth @quartzTiltAngle @lgTiltAngle @refOpeningAngle @refLength @lgLength @zRing @detWidth @pmtWindowSize @wallThick @wtOverlap $x $y $z $i $j $k);
...
$numRing = 7;    # number of rings to draw
@quartzTiltAngle = (45, 0, 0, 0, 0, 0, 0); # angle of tilt of quartz with respect to xy-plane
...
@quartzZ = (0, 400, 800, 1200, 1400, 1600, 1800); # position of the quartz ring along z-axis. Positive implies downstream. 
...
@lgTiltAngle = (25, 25, 35, 15, 0, -15, -20); # light guide tilt angle with respect to quartz piece. 
...
for $i(0..$numDetPerRing[$j]-1){
if($i!=0) {print def "                                              ";} 
print def "$quartzHeight[$j] $quartzWidth[$j] $quartzThickness[$j]";
if($i==$numDetPerRing[$j]-1){ print def "\"/>\n\n";  }
else { print def "\n";}
}
}
##------------------------------------------------##
...
print def "<box name=\"quartzRec_$j\_$i\" x=\"quartzDim_$j\[$i,1\]\" y=\"quartzDim_$j\[$i,2\]\" z=\"quartzDim_$j\[$i,3\]\" lunit= \"mm\" />\n";
...
print def "<union name=\"quartzBol_$j\_$i\">
    ...
    <position name=\"quartzCutPos_$j\_$i\" unit=\"mm\" x=\"quartzDim_$j\[$i,1\]/2+quartzDim_$j\[$i,3\]*tan(abs(lgTiltAngle[$j+1]*pi/180))/2\" y=\"0\" z=\"0\"/>
    <rotation name=\"quartzCutRot_$j\_$i\" unit=\"rad\" x=\"0\" y=\"pi/2\" z=\"0\"/> 
</union>\n"; 
print def "<union name=\"quartzDol_$j\_$i\">
    <first ref=\"quartzBol_$j\_$i\"/>
    <second ref=\"refOuterSol_$j\_$i\"/>
    <position name=\"quartzCutDolPos_$j\_$i\" unit=\"mm\" x=\"quartzDim_$j\[$i,1\]/2+quartzDim_$j\[$i,3\]*tan(abs(lgTiltAngle[$j+1]*pi/180))/2+refLength[$j+1]*cos(lgTiltAngle[$j+1]*pi/180)/2\" y=\"0\" z=\"refLength[$j+1]*sin(lgTiltAngle[$j+1]*pi/180)/2\"/>
    <rotation name=\"quartzCutDolRot_$j\_$i\" unit=\"rad\" x=\"0\" y=\"pi/2-lgTiltAngle[$j+1]*pi/180\" z=\"0\"/> 
</union>\n"; 
} else {
print def "<union name=\"quartzDol_$j\_$i\">
    <first ref=\"quartzRec_$j\_$i\"/>
...
print def "<union name=\"quartzSol_$j\_$i\">
    ...
    <position name=\"quartzCutSolPos_$j\_$i\" unit=\"mm\" x=\"quartzDim_$j\[$i,1\]/2+quartzDim_$j\[$i,3\]*tan(abs(lgTiltAngle[$j+1]*pi/180))/2+(2*refLength[$j+1]+1*lgLength[$j+1])*cos(lgTiltAngle[$j+1]*pi/180)/2\" y=\"0\" z=\"(2*refLength[$j+1]+1*lgLength[$j+1])*sin(lgTiltAngle[$j+1]*pi/180)/2\"/>
    <rotation name=\"quartzCutSolRot_$j\_$i\" unit=\"rad\" x=\"0\" y=\"pi/2-lgTiltAngle[$j+1]*pi/180\" z=\"0\"/> 
</union>\n"; 
	...
	<solidref ref=\"quartzSol_$j\_$i\"/> 
        <physvol name=\"det_$j\_$i\">
         ...
         <physvol name=\"ref_$j\_$i\">
			...
			<position name=\"refPos_$j\_$i\" unit=\"mm\" x=\"quartzDim_$j\[$i,1\]/2+quartzDim_$j\[$i,3\]*tan(abs(lgTiltAngle[$j+1]*pi/180))/2+refLength[$j+1]*cos(lgTiltAngle[$j+1]*pi/180)/2\" y=\"0\" z=\"refLength[$j+1]*sin(lgTiltAngle[$j+1]*pi/180)/2\"/>
			<rotation name=\"refRot_$j\_$i\" unit=\"rad\" x=\"0\" y=\"-pi/2+lgTiltAngle[$j+1]*pi/180\" z=\"0\"/>
         </physvol> 
          <physvol name=\"lg_$j\_$i\">
			<volumeref ref=\"lgVol_$j\_$i\"/>
			<position name=\"lgPos_$j\_$i\" unit=\"mm\" x=\"quartzDim_$j\[$i,1\]/2+quartzDim_$j\[$i,3\]*tan(abs(lgTiltAngle[$j+1]*pi/180))/2+(2*refLength[$j+1]+1*lgLength[$j+1])*cos(lgTiltAngle[$j+1]*pi/180)/2\" y=\"0\" z=\"(2*refLength[$j+1]+1*lgLength[$j+1])*sin(lgTiltAngle[$j+1]*pi/180)/2\"/>
			<rotation name=\"lgRot_$j\_$i\" unit=\"rad\" x=\"0\" y=\"-pi/2+lgTiltAngle[$j+1]*pi/180\" z=\"0\"/>
         </physvol> 
</volume>\n";
...
print def "<physvol name=\"quartzDet_$j\_$i\">
			...
			<position name=\"quartzPos_$j\_$i\" unit=\"mm\" x=\"quartzPos_$j\[$i,1\]*cos(quartzPos_$j\[$i,2\])\" y=\"quartzPos_$j\[$i,1\]*sin(quartzPos_$j\[$i,2\])\" z=\"quartzPos_$j\[$i,3\]\"/>
...
print def "\n</structure>\n
<setup name=\"logicDet\" version=\"1.0\">
